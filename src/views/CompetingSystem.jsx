...
function Group3Workflow({ category, participations, teams, scores, setScores, lang, showToast }) {
  ...
  // For AI Innovation, split teams into HS/US and ES/MS buckets shown as tables.
  const isAi = category.id === 'c3_ai';
  const buckets = isAi ? [
    { key: 'HS / US', divs: ['HS', 'US'], color: 'bg-purple-500' },
    { key: 'ES / MS', divs: ['ES', 'MS'], color: 'bg-brand-500' },
  ] : null;

  const renderTeamRow = (p) => {
    const tm = teams.find(team => team.id === p.teamId);
    if (!tm) return null;
    const teamScores = scores.filter(s => s.pId === p.id);
    const hasScore = teamScores.length > 0;
    const isSelected = selectedP === p.id;
    return (
      <button key={p.id} type="button" onClick={() => setSelectedP(isSelected ? '' : p.id)}
        className={`w-full flex items-center justify-between gap-3 p-3 rounded-xl border-2 text-left transition ${
          isSelected
            ? 'border-brand-500 bg-brand-50'
            : 'border-ink-200 bg-white hover:border-brand-300 hover:bg-brand-50/40'
        }`}>
        <div className="flex items-center gap-3 min-w-0">
          <span className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-ink-100 text-ink-700">{p.id}</span>
          <div className="min-w-0">
            <div className="font-semibold text-sm text-ink-800 truncate">{tm.name}</div>
            <div className="text-xs text-ink-500">{tm.division}{tm.teamNumber ? ` · #${tm.teamNumber}` : ''}</div>
          </div>
        </div>
        <span className={`text-[11px] font-bold px-2 py-1 rounded-md ${hasScore ? 'bg-emerald-100 text-emerald-700' : 'bg-ink-100 text-ink-500'}`}>
          {hasScore ? `${teamScores.length} ${tx('attempt(s)', 'محاولة')}` : tx('No score', 'لا توجد درجة')}
        </span>
      </button>
    );
  };
  ...
  return (
    <div className="space-y-5">
      ...
      {isAi ? (
        <div className="space-y-4">
          {buckets.map(({ key, divs, color }) => {
            const rows = filteredParticipations.filter(p => {
              const tm = teams.find(team => team.id === p.teamId);
              return tm && divs.includes(tm.division);
            });
            return (
              <CollapsibleCard key={key} title={`${tx('AI Innovation', 'الابتكار في الذكاء الاصطناعي')} — ${key}`} badge={rows.length} badgeColor={color}>
                {rows.length === 0 ? (
                  <div className="text-center py-6 text-ink-500 text-sm">{tx('No teams in this bucket.', 'لا توجد فرق في هذه المجموعة.')}</div>
                ) : (
                  <div className="space-y-2">{rows.map(renderTeamRow)}</div>
                )}
              </CollapsibleCard>
            );
          })}
